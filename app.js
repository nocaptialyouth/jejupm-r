(() => {
  ...
  function loadFavorites() {
    try {
      return JSON.parse(window.localStorage?.getItem("jeju-rehab-favorites") || "[]").map(String);
    } catch {
      return [];
    }
  }
...
* 관련 유튜브: ${youtubeUrl(item)}`;
  ...
  function saveFavorites() {
    try {
      window.localStorage?.setItem("jeju-rehab-favorites", JSON.stringify([...state.favorites]));
    } catch {
      // 로컬 파일 보안 설정 우회
    }
  }
  ...
  function renderDistrictRow() {
    ...
    allChip.innerHTML = `제주 전체 <span class="cnt" style="margin-left: 4px; font-size: 11px; opacity: 0.7;">${data.length}</span>`;
